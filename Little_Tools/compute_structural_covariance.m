...
%% ======================Input end======================
% Whole brain mask
[mask_whole_brain_data, header] = y_Read(mask_whole_brain);
mask_whole_brain_data = reshape(mask_whole_brain_data, 1,[]);
mask_whole_brain_data = mask_whole_brain_data ~= 0;

% Built SC
[~, coef_real_group1_pre, coef_rand_group1_pre, ~] =compute_structural_covariance_one_group(group1_pre, mask_seed, mask_whole_brain, ...
    n_perm, fullfile(out_path, 'group1_pre'));

[~, coef_real_group1_post, coef_rand_group1_post, ~]= compute_structural_covariance_one_group(group1_post, mask_seed, mask_whole_brain, ...
    n_perm, fullfile(out_path, 'group1_post'));

[~, coef_real_group2_pre, coef_rand_group2_pre, ~]= compute_structural_covariance_one_group(group2_pre, mask_seed, mask_whole_brain, ...
    n_perm, fullfile(out_path, 'group2_pre'));

[~, coef_real_group2_post, coef_rand_group2_post, ~]= compute_structural_covariance_one_group(group2_post, mask_seed, mask_whole_brain, ...
...
pvalues_group1_full =zeros(size(mask_whole_brain_data));
pvalues_group1_full(mask_whole_brain_data) = pvalues_group1;
pvalues_group1_full = reshape(pvalues_group1_full, header.dim);
y_Write(pvalues_group1_full, header, fullfile(out_path, 'pvalues_post_vs_pre_group1.nii'));
...
pvalues_group2_full =zeros(size(mask_whole_brain_data));
pvalues_group2_full(mask_whole_brain_data) = pvalues_group2;
pvalues_group2_full = reshape(pvalues_group2_full, header.dim);
y_Write(pvalues_group2_full, header, fullfile(out_path, 'pvalues_post_vs_pre_group2.nii'));
clear pvalues_group1 real_diff_group1 rand_diff_group1 pvalues_group1_pre
...
pvalues_inter_full =zeros(size(mask_whole_brain_data));
pvalues_inter_full(mask_whole_brain_data) = pvalues_inter;
pvalues_inter_full = reshape(pvalues_inter_full, header.dim);
y_Write(pvalues_inter_full, header, fullfile(out_path, 'pvalues_inter.nii'));
...
pvalues_inter_pre_full =zeros(size(mask_whole_brain_data));
pvalues_inter_pre_full(mask_whole_brain_data) = pvalues_inter_pre;
pvalues_inter_pre_full = reshape(pvalues_inter_pre_full, header.dim);
y_Write(pvalues_inter_pre_full, header, fullfile(out_path, 'pvalues_single_effect_pre.nii'));
...
pvalues_inter_post_full =zeros(size(mask_whole_brain_data));
pvalues_inter_post_full(mask_whole_brain_data) = pvalues_inter_post;
...
% Run
[pvalues_full_fwe, coef_real, coef_rand] = compute_structural_covariance_base(data_all, mask_seed, mask_whole_brain_data, n_perm);
...
coef_real_full(mask_whole_brain_data) = coef_real;
...
function [pvalues_fwe, coef_real, coef_rand] = compute_structural_covariance_base(data_all, mask_seed, mask_whole_brain_data, n_perm)
...
% Corr
[coef_real] = corr(intensity_in_seed_mean, intensity_in_whole_brain_intensity_mask);
coef_real(isnan(coef_real)) = 0;
% coef_real =zeros(size(mask_whole_brain_data));
% coef_real(mask_whole_brain_data) = coef;

% Bootstrap test for correlation analysis.
% n_boot = 1000;
coef_rand = zeros(n_perm, sum(mask_whole_brain_data));
n_greater = 0;
for i = 1 : n_perm
    rand_idx = randperm(size(data_all,1));
    [coef_rand] = corr(intensity_in_seed_mean(rand_idx,:), intensity_in_whole_brain_intensity_mask);
    coef_rand(i, :) = coef_rand;
    n_greater =  n_greater + (coef_real < max(abs(coef_rand)));  % FWE correction
end
pvalues_ttest1_fwe = (n_greater + 1) / (n_perm + 1);
coef_rand(isnan(coef_rand)) = 0;
