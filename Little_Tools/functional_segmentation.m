...
for i = 1: n_sub
    ...
    %     data_4d{i} = y_Read(subdata_path(1));
    datafile_path(i) = one_data_path(1);
end

%% Main
num_target_region = length(unique(target_region_id));
mask_target = arrayfun(@ (id)(atalas == id),target_region_id, 'UniformOutput',false);
mask_target_all = 0;
for i = 1:num_target_region
...
for i = 1:n_sub
    ...
    network_index_excluded_target_id = setdiff(network_index, target_region_id);
    atalas_combined = arrayfun(@(id)(atalas == id), network_index_excluded_target_id, 'UniformOutput',false);
    num_id = length(network_index_excluded_target_id);
    atalas_combined_all = 0;
    for j = 1: num_id
        atalas_combined_all = atalas_combined_all + atalas_combined{j}.* network_index_excluded_target_id(j);
    end
    
    average_signals_other_regions = y_ExtractROISignal_copy(datafile_path{i}, {atalas_combined_all},[], atalas, 1);
    ...
    coef = zeros(num_id, size(signals_of_target_in_the_region,2));
    for j = 1 : num_id
...
for j = 1: size(signals_of_target_in_the_region,2)
    segmentation(j) = find(coef(:,j) == coef_max(j));
end
