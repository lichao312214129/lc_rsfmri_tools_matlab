function lc_check_headmovement()
[FD_path, rigidbody_path, all_name]  = select_files();

FD_mov_thresh = 0.2;
proportion_thresh = 0.3;
rigidbody_thresh = 3;
savepath = 'F:\Data\Doctor';
check_headmovement(FD_path, rigidbody_path, all_name, ...
    FD_mov_thresh, proportion_thresh, rigidbody_thresh, savepath);

fprintf('Done\n');
end

function [FD_path, rigidbody_path, all_name]  = select_files(regstr_FD)
% Goal:Extract FD and rigid body movement files' path using regular expression
%%
% read all subj path
...
function check_headmovement(FD_path, rigidbody_path, all_name, FD_mov_thresh, proportion_thresh, rigidbody_thresh, savepath)
% Goal:
%   1:Calculate mean FD
...
for i = 1:n_sub
    ...
    meanFD(i) = mean(FD);
    if (meanFD(i) > 0.2) || (proportion_of_bad_timepoints(i) > proportion_thresh)
...
xlswrite(fullfile(savepath, 'headmovement.xlsx'),...
    ...
    'max_translation_x','max_translation_y','max_translation_z'...
    'max_rotation_x','max_rotation_y','max_rotation_z'},1, 'A1');
