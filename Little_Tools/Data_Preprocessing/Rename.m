...
%本代码只适用于有后缀的情况，例如张三.dcm。因为代码中涉及到读取‘.’。
...
switch Saveoldname
    case 'Y'
        ...
        for k=star:len
            ...
            if strcmp(object,'folder')
                newname=strcat(filename,'_',oldname,'_',int2str(k-2));
            else
                newname=strcat(filename,'_',oldname(1:find(oldname=='.')-1),int2str(k),oldname(find(oldname=='.'):end));
            end
    ...
    case 'N'
        ...
        for k=star:len
            ...
            if strcmp(object,'folder')
                newname=strcat(filename ,int2str(k-2));
            else
