...
%       ?? I think it should read: total E=n !!
%
%
% INPUTS:
%
...
function [wave,period,scale,coi] = ...
	wavelet_n(Y,dt,pad,dj,s0,J1,mother,param);
...
if (s0 == -1), s0=2*dt; end % smallest resolvable scale
if (dj == -1), dj = 1./4.; end % Morlet: choose <0.5
if (J1 == -1), J1=floor((log(n1*dt/s0)/log(2))/dj); end % [Eqn(10)], = log2(n1*dt/so)/dj
...
% k = frequency index in DFT (0:n-1)
k = [1:fix(n/2)];
k = k.*((2.*pi)/(n*dt)); % k<=N/2
k = [0., k, -k(fix((n-1)/2):-1:1)]; % k>N/2

%....compute FFT of the (padded) time series
f = fft(x);    % [Eqn(3)]
% will lead to errors at beginning & end of wavelet spectrum

%....construct SCALE array & empty PERIOD & WAVE arrays
scale = s0*2.^((0:J1)*dj); % [Eqn(9)] / nonortho wavelet: arbitrary set of scales, powers of 2 convenient
period = scale;
...
if (strcmp(mother,'MORLET'))  %-----------------------------------  Morlet
	...
	k0 = param;
	expnt = -(scale.*k - k0).^2/2.*(k > 0.);        % Table I
	norm = sqrt(scale*k(2))*(pi^(-0.25));%*sqrt(n);    % total energy at scale s=n   [Eqn(7)] -> NL: comment out sqrt(n) so total energy at scale is =1
    % k(2) = 2*pi/n/dt
