function [ PER,Accuracy, Sensitivity, Specificity, PPV, NPV, Decision, AUC, W_M_Brain,W_M_Brain_3D] =...
    SVM_LC_Kfold_ttest2_ByFeatureNum_beta(K,Initial_FeatureNum,Max_FeatureNum,Step_FeatureNum)
...
PER=[];%不同特征数目的平均分类表现
Num_loop=length(1:Step_FeatureNum:Max_FeatureNum);
W_M_Brain=zeros(Num_loop,dim1*dim2*dim3);

%===================================
j=0;%计数，为W_M_Brain赋值。
for N_feature=Initial_FeatureNum:Step_FeatureNum:Max_FeatureNum % 不同特征数目情况下
%% 预分配空间
...
W_M_Brain(j,implicitmask)=W_mean;%不同特征数目时的全脑体素权重
...
N_plot=length(Initial_FeatureNum:Step_FeatureNum:Max_FeatureNum);
h=figure;
h.Name='Mean Performance';
for j=1:6
subplot(3,2,j);plot((Initial_FeatureNum:Step_FeatureNum:Max_FeatureNum),PER(j,(1:2:2*N_plot)),'-');title(['Mean',' ',Name_plot{j}]);grid on;hold on;
end
g=figure;
g.Name= 'Std Performance';
for j=1:6
subplot(3,2,j);plot((Initial_FeatureNum:Step_FeatureNum:Max_FeatureNum),PER(j,[2:2:2*N_plot]),'-');title( ['Std',' ',Name_plot{j}]);grid on;hold on;
end
end
