function [ feature_ranking_list ] = FeatureSelection_RFE_SVM_Regression( data,label,opt )
...
%setting opt
if nargin <= 2
    opt.learner='leastsquares';%最小二乘线性回归
    opt.step=5;
    % iterations
    data_temp=data;
    [~,n]=size(data_temp);
...
%         disp([num2str(N-n+1),'/',num2str(N)]);
        Mdl= fitrlinear(data_temp,label,'Learner',opt.learner);%step 1: training a svm model
