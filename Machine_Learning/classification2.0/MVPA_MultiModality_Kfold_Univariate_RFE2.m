...
if ~exist('opt','var') && nargin < 3
    ...
    opt.IfLoadIndices=0;
    % standardization
    opt.standardizationMethod='normalization';
    opt.min_scale=0;
    opt.max_scale=1;
...
if nargin<2 && ~exist('dataMat','var')
    [fileName,folderPath,dataCell] =lc_Img2Data_MultiGroup('on',opt.numOfFeatureType*opt.numOfGroup);
    path=folderPath{1};
...
for i=1:opt.K
    ...
    % step2： 标准化或者归一化
    [dataTrain,dataTest]=lc_standardization(dataTrain,dataTest, opt.standardizationMethod);
    % step3：加入单变量的特征过滤
