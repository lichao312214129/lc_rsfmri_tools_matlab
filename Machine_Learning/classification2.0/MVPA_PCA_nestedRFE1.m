function [AUC,Accuracy,Sensitivity,Specificity] =...
    MVPA_PCA_nestedRFE1(opt,dataMat,label)
...
% new:
%         确定nested RFE K-fold中出现频率较高特征，再在外层训练集中以这些特征训练模型
...
for i=1:opt.K
    ...
    model= classifier(dataTrain(:,featureSelected),labelTrain);
    [labelPredict{i}, dec_values] = predict(model,dataTest(:,featureSelected));
...
%     weightOfFeature= weightOfComponent' * COEFF(:,featureSelected)';
