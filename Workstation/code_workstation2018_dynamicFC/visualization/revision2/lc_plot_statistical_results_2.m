...
%% INPUTS
results_root = 'D:\WorkStation_2018\WorkStation_dynamicFC_V3\Data\results\windowlength17__silhoutte_and_davies-bouldin\daviesbouldin\610_v2';
...
axes(ax(1)) 
lc_netplot('-n', state1, '-ni', net_index_path,'-lw',0.2, '-il',1, '-lg', legends, '-lgf', 4);
...
axes(ax(2)) 
lc_netplot('-n', state2, '-ni', net_index_path,'-lw',0.2, '-il',1, '-lg', legends, '-lgf', 4);
colormap(cmp_net0point5_pospoint8)
%     colorbar;
caxis([-0.5,0.8])
...
axes(ax(3)) 
lc_netplot('-n', state3, '-ni', net_index_path,'-lw',0.2, '-il',1, '-lg', legends, '-lgf', 4);
colormap(cmp_net0point5_pospoint8)
%     colorbar;
caxis([-0.5,0.8])
...
axis square
cb = colorbar('horiz','position',[0.35 0.15 0.28 0.02]); % ÏÔÊ¾colorbar
get(cb, 'YTick')
set(cb, 'YTick', [-0.5 0 0.5 0.8])
set(cb,'YTickLabel',{'-0.5','0','0.5','0.8'})
ylabel(cb,'Functional connectivity (Z)', 'FontSize', 10);  % ÉèÖÃcolorbarµÄtitle
