...
if nargin < 1
   ...
   design_matrix = cat(2, group_design_matched, cov_matched(:,[3,4,5,6]));
end
%% ancova
perms = 0;
GLM.perms = perms;
...
if is_save
    disp('save results...');
    save (fullfile(save_path,['sfc_STATS_results_',correction_method,'.mat']),'y_name','Fvalues','Pvalues','H');
    disp('saved results');
end
fprintf('--------------------------All Done!--------------------------\n');
end