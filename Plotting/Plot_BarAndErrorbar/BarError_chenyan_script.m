function BarError_chenyan_script( data_1,data_2,data_3)
%此函数用来绘制带有误差的柱状图。
%input:
...
%类似的输入对照组的变量矩阵
x = 1:size(data_1,2);
Mean=[mean(data_1,1);mean(data_2,1);mean(data_3,1)]';%均值。
Std=[std(data_1,1);std(data_2,1);std(data_3,1)]';
h = bar(x,Mean,0.4);
...
ax = gca;
ax.XTickLabels = {'左前扣带回','左脑岛','左中央后回','左丘脑','右前扣带回','右脑岛','右中央后回','右丘脑'};%改为你需要的横坐标名称，比如age/education等等
...
ylabel('均数 ± 标准差','FontName',' ','FontSize',20);
