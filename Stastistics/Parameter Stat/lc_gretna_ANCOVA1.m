function [ANCOVA_F, ANCOVA_P] = lc_gretna_ANCOVA1(DependentFiles, Covariates)
% 注意：该代码绝大多数部分来自gretna和dpabi软件，请引用！
...
for i=1:GroupNumber
    if ischar(DependentFiles{i})
        Matrix=importdata(DependentFiles{i});
...
[~, ~, ANCOVA_F, ~] = lc_gretna_GroupAnalysis(DependentMatrix, Regressors, Contrast, 'F');
